---
title: "【GLM-5】Claude CodeやCursorと同等性能で月$10〜$72。コスト1/3でAIコーディングする方法"
emoji: "💸"
type: "tech"
...
- 💸 Claude Max（月$100〜$200）と同等性能が、**GLM Coding Planなら月$10〜$72**で使える
...
そう感じている開発者に朗報です。2026年2月11日、中国の智譜AI（Z.ai）がリリースした**GLM-5**は、Claude Opus 4.5に迫るコーディング性能を持ちながら、**月$10〜$72の定額プラン**で使えます。年額契約ならさらに30%オフです。
...
| GPT-5.2 | 80.0% | - | $200 |
| **GLM-5** | **77.8%** | **56.2** | **$10〜$72** |
| Gemini 3.0 Pro | 73.2% | - | $19.99〜 |

GLM-5のスコア77.8%はClaude Opus 4.5の**96%の性能**です。Liteプラン（月$10）ならClaude Max 5x（月$100）の**1/10**、ヘビーユーザー向けMaxプラン（月$72）でもClaude Max 20x（月$200）の**1/3以下**。同じClaude Code上で使えてこの差額は大きいです。
...
つまり、今使っているツールを変える必要はありません。**バックエンドのモデルをGLM-5に差し替えるだけ**で、月額コストを大幅に削減できます。
...
| Windsurf | $15/月 + 従量課金 | |
| **GLM Coding Lite** | **≒$10/月** | **ライトユーザー向け** |
| **GLM Coding Pro** | **≒$27/月** | **本格利用（人気プラン）** |
| **GLM Coding Max** | **≒$72/月** | **ヘビーユーザー向け** |

:::message
**Claude Max 5xの$100/月に対し、GLM Coding Proは$27/月。同じClaude Code上で使えて約1/4のコスト。**
ライトユーザーならLite $10/月で**1/10**。年額契約ならさらに30%オフで**月$7〜$50**まで下がります。
:::

### GLM Coding Planの詳細

| プラン | 四半期 | 月額換算 | 年額（30%オフ） | 月額換算 | 5時間あたりの利用量 | GLM-5対応 |
|--------|--------|---------|----------------|---------|---------------------|-----------|
| **Lite** | $27 | ≒$9 | ≒$76 | ≒$6.3 | 約80プロンプト | 近日対応（GLM-4.7対応） |
| **Pro** | $81 | ≒$27 | ≒$227 | ≒$19 | 約400プロンプト | ✅ |
| **Max** | $216 | ≒$72 | ≒$605 | ≒$50 | 約1,600プロンプト | ✅ |

- 5時間ごとにプロンプト数がリセットされるサイクル制
- 使い切っても**追加課金は発生しない**（次のサイクルまで待てばOK）
- 予想外の高額請求が来ないので安心
- 2期目以降は四半期料金が10%オフ
- **年額契約なら30%オフ**（Pro年額 ≒ 月$19 = Claude Max $100/月の**約1/5**）

> ※2026年2月11日の値上げにより、以前の$3/月〜の初回割引は終了しています。既存ユーザーは旧料金が維持されます。
...
## まとめ：性能は同等、コストは最大1/10

GLM-5を一言でまとめると：

:::message
**Claude Opus 4.5の96%のコーディング性能を、1/4〜1/10の料金で使える。**
:::

- ✅ SWE-bench Verified 77.8%（OSS世界最高）
- ✅ Claude Code / Cursor / Cline / OpenCode など20以上のツール対応
- ✅ GLM Coding Plan Lite なら**月わずか$10**（年額なら月$6.3）
- ✅ Proプランでも月$27（年額なら月$19）でClaude Max $100の**約1/5**
- ✅ 5時間サイクル制で予想外の高額請求なし
- ✅ ハルシネーション率が業界最低水準
